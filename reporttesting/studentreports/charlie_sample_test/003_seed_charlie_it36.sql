-- Charlie Sample Test seed: IT36 (Subject ID 12, Class ID 19)
-- Seeds only attendance_logs table.
--
-- Existing data note (snapshot: databasedatafornow0329.txt):
--   - For Charlie (user_id=149), class_id=19 currently has 0 logs.
--   - Charlie has existing logs in class_id=24 (ENTRY=1, BREAK_OUT=1), not part of IT36.
--
...
-- WHERE user_id=149 AND class_id=19
-- GROUP BY 1,2,3
-- ORDER BY 1,2,3;
--
...
-- ======================
-- DIAGNOSTIC: Verify seeded sessions for Charlie IT36 (user_id=149, class_id=19)
-- ======================
SELECT
  al.user_id,
  al.class_id,
