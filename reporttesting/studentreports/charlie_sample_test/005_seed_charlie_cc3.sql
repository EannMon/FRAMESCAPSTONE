-- Charlie Sample Test seed: CC3 (Subject ID 14, Class IDs 22 and 23)
-- Seeds only attendance_logs table.
--
-- Existing data note (snapshot: databasedatafornow0329.txt):
--   - For Charlie (user_id=149), class_ids=22/23 currently have 0 logs.
--   - Charlie has existing logs in class_id=24 (ENTRY=1, BREAK_OUT=1), not part of CC3.
--
...
-- WHERE user_id=149 AND class_id IN (22,23)
-- GROUP BY 1,2,3,4
-- ORDER BY 1,2,3,4;
--
...
-- ======================
-- DIAGNOSTIC: Verify seeded sessions for Charlie CC3 (user_id=149, class_ids=22,23)
-- ======================
SELECT
  al.user_id,
  al.class_id,
