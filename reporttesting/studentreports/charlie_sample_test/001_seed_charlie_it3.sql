-- Charlie Sample Test seed: IT3 (Subject ID 9, Class IDs 15 and 16)
-- Seeds only attendance_logs table.
--
-- Existing data note (snapshot: databasedatafornow0329.txt):
--   - For Charlie (user_id=149), class_ids=15/16 are seeded to represent twice-weekly IT3 sessions.
--   - Charlie has existing logs in class_id=24 (ENTRY=1, BREAK_OUT=1), not part of IT3.
--
...
-- WHERE user_id=149 AND class_id IN (15,16)
-- GROUP BY 1,2,3
-- ORDER BY 1,2,3;
--
...
-- JOIN subjects s ON s.id = c.subject_id
-- LEFT JOIN users f ON f.id = c.faculty_id
-- JOIN users u ON u.id = 149
-- LEFT JOIN attendance_logs al ON al.class_id = c.id AND al.user_id = 149
