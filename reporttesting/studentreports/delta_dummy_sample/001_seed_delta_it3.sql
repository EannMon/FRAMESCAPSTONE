-- Delta Dummy Sample seed: IT3 (Subject ID 9, Class IDs 15 and 16)
-- Seeds only attendance_logs table.
--
-- Existing data note (snapshot: databasedatafornow0329.txt):
--   - For Delta (user_id=155), class_ids=15/16 are seeded to represent twice-weekly IT3 sessions.
--   - Delta has existing logs in class_id=24 (ENTRY=1, BREAK_OUT=1), not part of IT3.
--
...
-- WHERE user_id=155 AND class_id IN (15,16)
-- GROUP BY 1,2,3
-- ORDER BY 1,2,3;
--
...
-- JOIN subjects s ON s.id = c.subject_id
-- LEFT JOIN users f ON f.id = c.faculty_id
-- JOIN users u ON u.id = 155
-- LEFT JOIN attendance_logs al ON al.class_id = c.id AND al.user_id = 155
...
filtered_sessions AS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE existing_day.user_id = 155
...
SELECT
    ...
    CASE WHEN sp.scenario = 'EARLY_EXIT'
         ...
         ELSE sp.end_ts END,
    CASE WHEN sp.scenario = 'EARLY_EXIT' THEN '[SEED-DELTA-REPORTS] Early exit'
         WHEN sp.scenario = 'AUTO_EXIT_CASE' THEN '[SEED-DELTA-REPORTS] [AUTO_EXIT] System auto-exit simulation'
         ELSE '[SEED-DELTA-REPORTS]' END,
    FALSE
FROM session_plan sp
WHERE sp.scenario <> 'ABSENT_SIMULATION'
  AND NOT EXISTS (
