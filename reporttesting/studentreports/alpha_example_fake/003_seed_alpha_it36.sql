-- Alpha Example Fake seed: IT36 (Subject ID 12, Class ID 19)
-- Seeds only attendance_logs table.
--
-- Existing data note (snapshot: databasedatafornow0329.txt):
--   - For Alpha (user_id=177), class_id=19 currently has 0 logs.
--   - Alpha has existing logs in class_id=24 (ENTRY=1, BREAK_OUT=1), not part of IT36.
--
...
-- WHERE user_id=177 AND class_id=19
-- GROUP BY 1,2,3
-- ORDER BY 1,2,3;
--
...
SELECT
    ...
    CASE WHEN sp.scenario = 'AUTO_EXIT_CASE' THEN 0.0 ELSE 0.877 END,
