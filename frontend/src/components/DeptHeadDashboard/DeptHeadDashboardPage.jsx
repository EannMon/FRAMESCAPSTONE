...
// --- RIBBON STAT ITEM (clickable) ---
const DeptHeadStatItem = ({ title, value, subValue, onClick }) => (
    <div className={`fd-ribbon-item ${onClick ? 'dh-ribbon-clickable' : ''}`} onClick={onClick} style={onClick ? { cursor: 'pointer' } : {}}>
        <span className="fd-ribbon-label">{title}</span>
        <span className="fd-ribbon-value">{value}</span>
        {subValue && <span className="fd-ribbon-sub">{subValue}</span>}
    </div>
);
        ...
        <div className="fd-card fd-chart-card">
            <div className="fd-card-header">
                ...
                <div className="fd-chart-controls">
                    <select value={trendView} onChange={(e) => setTrendView(e.target.value)} className="fd-select">
            ...
            <div className="fd-svg-container">
                <svg viewBox={`0 0 ${width} ${height}`} className="fd-trend-svg">
                    ...
                    {(typeFilter === 'ALL' || typeFilter === 'LATE') &&
                        <path d={`${makePath('late')} L ${width - padding},${height - padding} L ${padding},${height - padding} Z`} fill="url(#dhGradLate)" stroke="none" />}
                    {(typeFilter === 'ALL' || typeFilter === 'BREAK') &&
                        <path d={`${makePath('break')} L ${width - padding},${height - padding} L ${padding},${height - padding} Z`} fill="url(#dhGradBreak)" stroke="none" />}
                    {(typeFilter === 'ALL' || typeFilter === 'PRESENT') &&
                        <path d={`${makePath('present')} L ${width - padding},${height - padding} L ${padding},${height - padding} Z`} fill="url(#dhGradPresent)" stroke="none" />}

                    {(typeFilter === 'ALL' || typeFilter === 'LATE') &&
                        <path d={makePath('late')} fill="none" stroke={colors.late} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" filter="url(#dhLineShadow)" />}
                    {(typeFilter === 'ALL' || typeFilter === 'BREAK') &&
                    ...
                    {(typeFilter === 'ALL' || typeFilter === 'PRESENT') &&
                        <path d={makePath('present')} fill="none" stroke={colors.present} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" filter="url(#dhLineShadow)" />}
                            ...
                            <g key={i} onMouseEnter={() => setHoveredIndex(i)} onMouseLeave={() => setHoveredIndex(null)}>
                                ...
                                {hoveredIndex === i && (
                                    <g transform={`translate(${xp}, 20)`}>
                                        <rect x="-60" y="-10" width="120" height="70" rx="5" fill="var(--chart-tooltip-bg, white)" filter="url(#dhTooltipShadow)" stroke="var(--chart-tooltip-border, #e2e8f0)" />
...
// --- LIVE STATUS (3 views: Personal, Classroom, Department) ---
const DeptHeadLiveStatus = ({ rooms, personalStatus }) => {
    const [viewMode, setViewMode] = useState('wide');
    const [selectedRoom, setSelectedRoom] = useState(null);
    const [statusView, setStatusView] = useState('department');
...
const RecentActivity = ({ activities }) => {
    ...
    return (
        <div className="fd-card fd-activity-card dh-activity-full">
            <div className="fd-card-header">
        ...
        <div className="fd-page">
            ...
            <div className="fd-hero-section">
                    ...
                    <div className="fd-hero-content">
                        <div className="fd-hero-left">
                            <div className="fd-hero-text">
                                ...
                                <h2 className="fd-hero-name">{displayName}</h2>
                                <p className="fd-hero-id">Department Head • {user?.college_name || 'COS'}</p>
                            </div>
                        </div>
                        <div className="fd-hero-right">
                            <div className="fd-hero-date">
                ...
                <div className="fd-stats-ribbon">
                    ...
                    <DeptHeadStatItem
                        title="My Students"
                        value={stats.total_teaching_students || 0}
                        subValue="Teaching"
            ...
            {!faceRegistered && (
                <div className="dh-face-warning fd-card">
                    <div className="dh-warning-content">
                        <strong>Face Registration Required</strong>
                        <p>
                            FRAMES requires facial recognition enrollment before you can fully access the system.
                            Please visit a registration kiosk or use the face enrollment feature in <strong>Settings</strong>.
                        </p>
                    </div>
                    <button onClick={() => navigate('/head-settings')} className="dh-warning-btn">
                        Go to Settings
                    </button>
                </div>
            )}
